...
RUN tar xf openssh-${VERSION}.tar.gz && \
    cp ./openssh-${VERSION}/contrib/redhat/openssh.spec /home/rpmbuilder/rpmbuild/SPECS/ && \
...
RUN rpmbuild  -bb  openssh.specRUN sed  -i   's/^#%__check_files/%__check_files/g'     /usr/lib/rpm/macros
#RUN cp /home/rpmbuilder/rpmbuild/RPMS/x86_64/*.rpm /tmp/
